...
# ============================================================
# INDIVIDUAL MCP SERVERS (Thin wrappers)
# ============================================================
...
class FileSystemMCPServer:
    ...
    def __init__(self, allowed_paths: list[str] | None = None):
        ...
        self.manager = FileSystemManager(allowed_paths)
    ...
    def _setup_handlers(self):
        ...
        async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
            if name == "read_file":
                result = self.manager.read_file(arguments["path"], arguments.get("max_lines"))
            elif name == "list_directory":
                result = self.manager.list_directory(arguments["path"], arguments.get("pattern", "*"))
            elif name == "get_file_info":
                result = self.manager.get_file_info(arguments["path"])
            else:
...
class DatabaseMCPServer:
    ...
    def __init__(self, db_connection: sqlite3.Connection | None = None):
        ...
        self.manager = DatabaseManager(db_connection or init_demo_database())
        self._setup_handlers()
    ...
    def _setup_handlers(self):
        ...
        async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
            if name == "query_products":
                result = self.manager.query_products(**arguments)
            elif name == "query_sales":
                result = self.manager.query_sales(**arguments)
            elif name == "get_analytics":
                result = self.manager.get_analytics(arguments["metric"])
            else:
                result = {"error": f"Unknown tool: {name}"}
...
class ActionsMCPServer:
    ...
    def __init__(self, output_dir: str | None = None):
        ...
        self.manager = ActionsManager(Path(output_dir or "./output"))
        self._setup_handlers()
    
    def _setup_handlers(self):
        @self.server.list_tools()
        async def list_tools() -> list[Tool]:
            return get_actions_tools()
        
        @self.server.call_tool()
        async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
            if name == "generate_report":
                result = self.manager.generate_report(**arguments)
            elif name == "send_notification":
                result = self.manager.send_notification(**arguments)
            elif name == "create_task":
                result = self.manager.create_task(**arguments)
            elif name == "get_action_log":
                result = self.manager.get_action_log(arguments.get("action_type", "all"))
            else:
                result = {"error": f"Unknown tool: {name}"}
...
class AggregatorMCPServer:
    ...
    def __init__(self):
        self.server = Server("aggregator-server")
        self.manager = AggregatorManager()
        self._setup_handlers()
    
    def _setup_handlers(self):
        @self.server.list_tools()
        async def list_tools() -> list[Tool]:
            return get_aggregator_tools()
        
        @self.server.call_tool()
        async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
            if name == "aggregate_for_chart":
                ...
                result = self.manager.aggregate_for_chart(
                    data,
                    arguments["group_by"],
...
class GrapherMCPServer:
    """MCP Server for data visualization."""
    
    def __init__(self, output_dir: str | None = None):
        self.server = Server("grapher-server")
        self.manager = GrapherManager(Path(output_dir or "./output"))
        self._setup_handlers()
    
    def _setup_handlers(self):
        @self.server.list_tools()
        async def list_tools() -> list[Tool]:
            return get_grapher_tools()
        ...
        async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
            if name == "create_chart":
                ...
                result = self.manager.create_chart(
                    chart_data,
...
# ============================================================
# COMBINED MCP SERVER (No duplication!)
# ============================================================
...
async def run_aggregator_server():
    """Entry point for aggregator MCP server."""
    server = AggregatorMCPServer()
    await server.run()


async def run_grapher_server():
    """Entry point for grapher MCP server."""
    server = GrapherMCPServer()
    await server.run()
...
if __name__ == "__main__":
    ...
    elif server_type == "aggregator":
        asyncio.run(run_aggregator_server())
    elif server_type == "grapher":
        asyncio.run(run_grapher_server())
    else:
